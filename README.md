...
**RIGOL MHO98 系列示波器的 PC 上位机**——通过 Windows 原生 USBTMC 或 LAN 连接，实现双通道正弦波输出、0.1–100 Hz 线性扫频、CH1/CH2 波形采集与伯德图分析。
...
| 📈 **线性扫频** | `0.1–100 Hz` 共 300 个频点，按频段自动切换示波器时基 |
| 📊 **CH1 / CH2 采集** | 每个频点采集 7 个完整周期，仅保存中间第 2–6 周期，数据实时写入 CSV |
| 📉 **伯德图分析** | 正弦拟合后计算幅值比、增益 dB 与相位差，并给出 R² 与有效点数 |
| 🖼️ **波形预览** | 界面内置 7 周期设定波形预览，扫频进度实时可见 |
...
点击 **「开始扫频（0.1–100 Hz）」**，程序分三段生成 300 个严格递增、不重复的频点：

| 频段 | 频点数 | 示波器时基 | 每点保持时间 |
| --- | --- | --- | --- |
| 0.1 – <1 Hz | 100 | 10 s/div | `7 / f`（恰好 7 个周期） |
| 1 – <10 Hz | 100 | 1 s/div | `7 / f` |
| 10 – 100 Hz | 100 | 100 ms/div | `7 / f` |

- 时基仅在 1 Hz 与 10 Hz 频段边界切换；扫频结束或停止时自动恢复开始前的时基。
- 所有频点理论保持时间约 **33 分 43 秒**，实际总时长还需加上 300 次双通道波形传输与 CSV 写入时间。
- 运行期间再次点击扫频或记录按钮，会停止扫频并关闭当前 AFG 输出。

### 4️⃣ CH1 / CH2 电压采集

- 点击 **「开始记录」**（或直接开始扫频）后，先选择 CSV 保存位置，文件会立即在电脑上创建。
- 每个频点先运行完整 7 周期，再依次读取 CH1、CH2 屏幕原始波形；**仅第 2–6 周期写入 CSV**，第 1、7 周期丢弃。
...
- **「加载波形 CSV…」** 支持：
  - 本程序生成的完整 CSV；
  - 仅含 `frequency`、`CH1`、`CH2` 三列的简化 CSV（无时间列时按每频点等间隔 5 周期处理，推荐提供 `point_time_s` 或 `sample_interval_s` 以获得可靠相位）。
