"""Plot CH1/CH2 with identical user-configurable axes."""
...
def create_figure(
    data: pd.DataFrame,
    output_stem: Path,
    time_tick_interval: float,
    voltage_tick_interval: float,
    ...
    for axis, (column, label, color) in zip(axes, CHANNELS):
        ...
        axis.yaxis.set_major_locator(MultipleLocator(voltage_tick_interval))
        axis.yaxis.set_major_formatter(FormatStrFormatter("%.1f"))
        axis.set_ylabel("Voltage (V)")
        axis.grid(True, which="major", color="#D0D0D0", linewidth=0.6, alpha=0.8)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-stem",
        default="MHO98_CH1_CH2_equal_axes",
        help="output filename without an extension",
    )
    parser.add_argument("--time-tick", type=float, default=10.0, metavar="SECONDS")
    parser.add_argument("--voltage-tick", type=float, default=0.2, metavar="VOLTS")
    parser.add_argument(
        "--time-limits",
        type=float,
        nargs=2,
    ...
    if args.time_tick <= 0 or args.voltage_tick <= 0:
        parser.error("tick intervals must be greater than zero")
    if args.time_limits[0] >= args.time_limits[1]:
        parser.error("--time-limits MIN must be less than MAX")
    if args.voltage_limits[0] >= args.voltage_limits[1]:
        parser.error("--voltage-limits MIN must be less than MAX")

    time_limits = tuple(args.time_limits)
    voltage_limits = tuple(args.voltage_limits)
    data = load_data(args.csv_path)
    png_path, pdf_path = create_figure(
        data,
        args.output_dir / args.output_stem,
        args.time_tick,
        args.voltage_tick,
        time_limits,
        voltage_limits,
    ...
    print(f"Time tick interval: {args.time_tick:g} s")
    print(f"Voltage tick interval: {args.voltage_tick:g} V")
